    ...
    if (type === 'textarea') {
      ...
      return (
        <div style={this.style()} className={this.className(classname)} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
          ...
          { typeof icon === 'string' ? <i className={`el-input__icon mmspc-icon-${icon}`} onClick={this.handleIconClick.bind(this)}>{prepend}</i> : icon }
          <input { ...otherProps }
            ref="input"
            type={type}
            ...
            onBlur={this.handleBlur.bind(this)}
          />
          { validating && <i className="el-input__icon mmspc-icon-loading"></i> }
